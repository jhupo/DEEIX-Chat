...
Protected Browser routes validate the short-lived DEEIX access token against the persisted Principal session, then use the current database Principal role instead of the token role claim. The Principal role is refreshed from Sub2 `/auth/me`: Sub2 `admin` maps to DEEIX `superadmin`, and Sub2 `user` maps to DEEIX `user`.
