...
项目与对话是两个维度：Gateway 的 Project 来自 canonical `cwd`/Workspace，Conversation 来自 app-server thread。项目树按 Workspace 分组显示其中的对话；“最近”按 thread 更新时间显示当前执行上下文的全部对话，即使该对话属于某个项目也不从最近列表排除。前端不从标题猜测项目，也不为 Gateway 建第二套侧栏。
