...
DEEIX_AGENT_DATA_DIR=%s DEEIX_AGENT_HOME=%s sh %s --server %s --user %s --codex %s >>%s 2>&1
`, os.Getpid(), shellLiteral(wrapperPath), shellLiteral(installerPath), shellLiteral(dataDir), shellLiteral(filepath.Dir(executable)),
		shellLiteral(installerPath), shellLiteral(config.CloudURL), shellLiteral(config.UserPublicID), shellLiteral(config.CodexExecutable), shellLiteral(logPath))
