...
// Client 定义 application 层使用的模型调用能力。
type Client interface {
	Generate(context.Context, RouteConfig, GenerateInput) (*GenerateOutput, error)
	GenerateStream(context.Context, RouteConfig, GenerateInput, func(GenerateStreamEvent) error) (*GenerateOutput, error)
	ListModels(context.Context, RouteConfig) ([]ModelItem, error)
	RetrieveOpenAIResponse(context.Context, RouteConfig, string) (*GenerateOutput, error)
	CancelOpenAIResponse(context.Context, RouteConfig, string) (*GenerateOutput, error)
}
